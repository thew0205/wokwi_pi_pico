...
if [ -n "$MODEL_INPUT" ]; then
  ...
  cmake -B "${BUILD_DIR}" -DPICO_BOARD=pico -DINPUT_MODEL="./${MODEL_INPUT}" -DCMAKE_BUILD_TYPE=Release -GNinja
else
  # Use default model
  echo "Building firmware with default model: ${DEFAULT_MODEL}"
  cmake -B "${BUILD_DIR}" -DPICO_BOARD=pico -DINPUT_MODEL="./${DEFAULT_MODEL}" -DCMAKE_BUILD_TYPE=Release -GNinja
fi
